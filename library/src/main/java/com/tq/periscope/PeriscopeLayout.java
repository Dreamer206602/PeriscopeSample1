...
import com.tq.periscope.R;
import com.tq.periscope.BezierEvaluator;
...
public class PeriscopeLayout extends RelativeLayout {
    ...
    private void init() {
        ...
        //获取图片的宽和高，用于后面的计算
        //注意 在这里3张图片的大小都是一样的，所以只取了一个
        dHeight=red.getIntrinsicHeight();
        dWidth=red.getIntrinsicWidth();

        //底部 并且 水平居中
        lp=new LayoutParams(dWidth,dHeight);
        lp.addRule(CENTER_HORIZONTAL,TRUE);//在这里的TRUE 要注意 不是 true
     ...
     * @return
     */
    private Animator getEnterAnimator(final  View target){
    ...
    private PointF getPointF(int scale) {
        ...
        pointF.x=random.nextInt((mWidth-100));//减去100，是为了控制x轴的活动范围
        //在Y轴上，为了确保第二个点，在第一个点之上，我把Y分成了上下两半，这样的效果更好
    ...
    private  class BezierListener implements ValueAnimator.AnimatorUpdateListener{
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            //这里获取到贝塞尔曲线计算出来的x,y的值 赋值给view 这样就能让爱心随着曲线走啦
    ...
    private class AnimEndListener extends AnimatorListenerAdapter{
        ...
        public void onAnimationEnd(Animator animation) {
            ...
            //因为不停的add 导致子view只增不减，所以在view动画结束后remove掉
            removeView(target);
        }
    }
}
